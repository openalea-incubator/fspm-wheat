                    ...
                    for mtg_organ_vid in self._shared_mtg.components_iter(mtg_metamer_vid):
                        ...
                        if set(mtg_organ_properties).issuperset(simulation.ORGAN_INPUTS):
                            ...
                            for growthwheat_organ_input_name in simulation.ORGAN_INPUTS:
                                growthwheat_organ_inputs_dict[growthwheat_organ_input_name] = mtg_organ_properties[growthwheat_organ_input_name]
                            # temp
                            for mtg_element_vid in self._shared_mtg.components_iter(mtg_organ_vid):
                                if self._shared_mtg.get_vertex_property(mtg_element_vid)['label'] in ('StemElement', 'LeafElement1'):
                                    growthwheat_organ_inputs_dict['green_area'] = self._shared_mtg.get_vertex_property(mtg_element_vid)['green_area']
