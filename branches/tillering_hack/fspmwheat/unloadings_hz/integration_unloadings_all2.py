...
##
## Ici on utilise la dernière valeur de flux fournie par le solveur
##
    ...
    if os.stat(i_file).st_size != 0:
        ...
        for i_elt in map_elt.index:
            ...
            for i in df_elt.index:
                if i>1:
                    df_elt.at[i,'delta_t_int'] = df_elt.at[i-1,'t_int'] - df_elt.at[i-2,'t_int']
...
output_file = 'all_integration_unloadings2.csv'
...
res.to_csv(output_file, na_rep='NA', index=False)